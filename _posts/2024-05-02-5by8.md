...
<ins>**Theorem**</ins>. _Let $G$ be a finite group non-abelian group and $\mathbb{P}(G)$ denote the probability that two randomly selected (with replacement) elements in $G$ commute. Then_  $\displaystyle \mathbb{P}(G)\leq \frac{5}{8}.$

Equivalently, the inner automorphism group of $G$ is $\mathrm{Inn}(G) = \mathbb{Z}_2\times\mathbb{Z}_2,$ or 
$ |\mathrm{Inn}(G)|=4.$
