...
The great mathematician Poincar&eacute; actually classified all possible $2$-d shapes; they are classified (ignoring something called 'non-orientable' shapes) by a single number called the **genus**. The genus of a surface is the number of holes: a sphere has genus $0$ (no holes), but a torus (the surface of a donut) has genus $1$ (because it has $1$ hole, the donut-hole).
